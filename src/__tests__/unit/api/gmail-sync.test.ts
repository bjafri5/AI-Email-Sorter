...
import { classifyEmail, classifyAndSummarizeEmail } from "@/lib/ai";
...
describe("Gmail Sync Flow", () => {
  ...
  it("processes new emails, classifies, summarizes, and archives", async () => {
    ...
    // AI responses - using combined function
    vi.mocked(classifyAndSummarizeEmail).mockResolvedValue({
      categoryId: "cat1",
      summary: "Weekly tech newsletter summary",
    });
    vi.mocked(cleanEmailBody).mockReturnValue("This is a test newsletter");

    // Email creation
    vi.mocked(prisma.email.create).mockResolvedValue({ id: "email1" } as never);
    ...
    // Process email with combined function
  ...
  it("skips duplicate emails based on gmailId", async () => {
    ...
    expect(classifyAndSummarizeEmail).not.toHaveBeenCalled();
  });
  ...
  it("handles emails with no category match", async () => {
    ...
    const result = await classifyAndSummarizeEmail({} as never, []);
